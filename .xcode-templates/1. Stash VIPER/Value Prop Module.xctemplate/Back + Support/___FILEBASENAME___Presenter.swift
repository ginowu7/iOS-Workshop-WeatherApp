...
class ___VARIABLE_productName___Presenter {
    ...
    private func configureNavigation() {
        let navModel = NavigationViewModel.leftBackRightSupport(withTitle: "", leftAction: { [weak wireframe] in
            wireframe?.dismiss()
        }, rightAction: { [weak wireframe] in
            wireframe?.showSupportDialog()
        })

        userInterface.configureNavigation(with: navModel)
    }
